...
import { html, raw, esc, money, eur, sum, clamp, fmtDate, fmtDateShort } from './util.js';
...
const P = {
  ...
  // Zahnrad mit acht Zähnen, errechnet (Kopfkreis 9,5, Fußkreis 7,3, um die
  // Mitte gedreht) statt von Hand gesetzt: die frühere Fassung war schief und
  // ragte links aus dem Bildfeld.
  settings: '<path d="M10.36 4.89L10.6 2.6A9.5 9.5 0 0 1 13.4 2.6L13.64 4.89A7.3 7.3 0 0 1 15.87 5.81L17.65 4.36A9.5 9.5 0 0 1 19.64 6.35L18.19 8.13A7.3 7.3 0 0 1 19.11 10.36L21.4 10.6A9.5 9.5 0 0 1 21.4 13.4L19.11 13.64A7.3 7.3 0 0 1 18.19 15.87L19.64 17.65A9.5 9.5 0 0 1 17.65 19.64L15.87 18.19A7.3 7.3 0 0 1 13.64 19.11L13.4 21.4A9.5 9.5 0 0 1 10.6 21.4L10.36 19.11A7.3 7.3 0 0 1 8.13 18.19L6.35 19.64A9.5 9.5 0 0 1 4.36 17.65L5.81 15.87A7.3 7.3 0 0 1 4.89 13.64L2.6 13.4A9.5 9.5 0 0 1 2.6 10.6L4.89 10.36A7.3 7.3 0 0 1 5.81 8.13L4.36 6.35A9.5 9.5 0 0 1 6.35 4.36L8.13 5.81A7.3 7.3 0 0 1 10.36 4.89Z"/><circle cx="12" cy="12" r="3"/>',
  ...
  sort: '<path d="M8 20V5M4 9l4-4 4 4"/><path d="M16 4v15M12 15l4 4 4-4"/>',
  arrowUp: '<path d="M12 19V5M6 11l6-6 6 6"/>',
  arrowDown: '<path d="M12 5v14M6 13l6 6 6-6"/>',
  grip: '<circle cx="9" cy="6" r="1.2"/><circle cx="15" cy="6" r="1.2"/><circle cx="9" cy="12" r="1.2"/><circle cx="15" cy="12" r="1.2"/><circle cx="9" cy="18" r="1.2"/><circle cx="15" cy="18" r="1.2"/>',
  layout: '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 10h18M12 10v11"/>',
...
/**
 * Veränderung in Prozent. Die Farbe sagt, ob das gut ist: Bei Ausgaben
 * (`invert`) ist ein Anstieg rot, der Pfeil zeigt trotzdem nach oben.
 */
export function deltaBadge(fraction, { invert = false } = {}) {
  if (fraction === null || fraction === undefined || !Number.isFinite(fraction)) {
    return raw('<span class="delta flat">–</span>');
  }
  const rauf = fraction > 0.001;
  const runter = fraction < -0.001;
  const cls = rauf || runter ? ((rauf !== invert) ? 'up' : 'down') : 'flat';
...
/**
 * Beschriftung neben einer Veränderung zum Vorzeitraum (aus compareRanges).
 * Im laufenden Zeitraum mit Stichtag, damit klar ist, dass nur bis zum
 * gleichen Stand verglichen wird.
 */
export function compareLabel({ prevFrom, prevTo, toDate }) {
  const text = toDate ? `ggü. Vorzeitraum bis ${fmtDateShort(prevTo)}` : 'ggü. Vorzeitraum';
  const title = `Verglichen mit ${fmtDate(prevFrom)} – ${fmtDate(prevTo)}`
    + (toDate ? `, beide Zeiträume bis zum gleichen Stand (heute: ${fmtDate(toDate)})` : '');
