...
import { html, raw, esc, money, int, sum } from './util.js';
import { unlistedStats } from './calc.js';
import { chart, emptyState, segToggle, wireSeg } from './ui.js';
...
/** Der Verlauf in der gewählten Darstellung. Die Tabelle braucht danach mountTables(). */
export function verlaufBody(months, avg, labelFor) {
  if (prefs.verlauf === 'table') return monthTable(months, avg, labelFor);
 ...
 * Sortierbar, etwa um den stärksten Monat oben zu haben.
 */
function monthTable(series, avg, labelFor) {
  if (!series.length) return emptyState('Keine Daten', 'Für diesen Zeitraum gibt es keine Buchungen.');
  const geld = (key, tone = false) => (s) => {
    const v = s[key];
    return `<span class="${tone ? (v > 0 ? 'amount pos' : v < 0 ? 'amount neg' : '') : ''}">${esc(money(v))}</span>`;
  };
  const zelle = (v, tone = false) => `<td class="num"><span class="${tone ? (v > 0 ? 'amount pos' : v < 0 ? 'amount neg' : '') : ''}">${esc(money(v))}</span></td>`;
  return table({
    id: 'monate',
    cls: 'data compact',
    toolbar: false,
    defaultSort: { key: 'ym', dir: 1 },
    rows: series,
    columns: [
      { key: 'ym', label: 'Monat', type: 'date', dirText: ['chronologisch', 'neueste zuerst'], cell: (s) => esc(labelFor(s)) },
      { key: 'income', label: 'Einnahmen', type: 'num', cell: geld('income') },
      { key: 'expense', label: 'Ausgaben', type: 'num', cell: geld('expense') },
      { key: 'profit', label: 'Ergebnis', type: 'num', cell: geld('profit', true) },
    ],
    foot: () => `
      <tr><td>Summe</td>${zelle(sum(series, (s) => s.income))}${zelle(sum(series, (s) => s.expense))}${zelle(sum(series, (s) => s.profit))}</tr>
      ${avg && avg.months ? `<tr class="avg-row"><td>Ø je Monat <span class="muted tiny">(${avg.months} Monate)</span></td>${zelle(avg.income)}${zelle(avg.expense)}${zelle(avg.profit, true)}</tr>` : ''}`,
  });
}
