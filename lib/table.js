...
export function mountTable(host, spec) {
  ...
  const filterMenu = (btn) => {
    ...
    const fs = filters.filter((f) => f.column === col);
    openMenu(btn, {
      label: fs.map((f) => f.title).join(', '),
      align: btn.closest('th')?.classList.contains('num') || btn.closest('th')?.classList.contains('center') ? 'end' : 'start',
      sections: fs.map((f) => {
      ...
      onPick: (key, val) => {
        st.filters[key] = val;
        render(`[data-filter="${CSS.escape(col)}"]`);
      },
    });
  };
