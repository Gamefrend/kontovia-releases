...
/**
 * Beschriftung der Befehlstaste in Tastenkürzeln: ⌘ auf Apple-Geräten (die
 * Web-Fassung reagiert dort auf ⌘ wie unter Windows auf Strg), sonst Strg.
 */
export const MOD = /Mac|iPhone|iPad|iPod/.test(globalThis.navigator?.platform || '') ? '⌘' : 'Strg';

export function bytes(n) {
  const v = Number(n) || 0;
  // Nur für die Anzeige, deshalb mit deutschem Dezimalkomma.
  const komma = (x, stellen) => x.toFixed(stellen).replace('.', ',');
  if (v < 1024) return `${v} B`;
  if (v < 1024 ** 2) return `${komma(v / 1024, 1)} KB`;
  if (v < 1024 ** 3) return `${komma(v / 1024 ** 2, 1)} MB`;
  return `${komma(v / 1024 ** 3, 2)} GB`;
}
